...
describe("unsubscribe token", () => {
  it("round-trips a valid token to its email + sendId", () => {
    const t = makeUnsubToken("User@example.com", "abc123");
    const v = verifyUnsubToken(t);
    expect(v).not.toBeNull();
    expect(v!.email).toBe("user@example.com"); // normalized lowercase
    expect(v!.sendId).toBe("abc123");
  });

  it("rejects a tampered signature", () => {
    const t = makeUnsubToken("a@example.com", "s1");
    const [body] = t.split(".");
  ...
  it("rejects a tampered payload (email swap)", () => {
    const t = makeUnsubToken("a@example.com", "s1");
    const sig = t.split(".")[1];
    const forged = Buffer.from(JSON.stringify({ e: "evil@example.com", s: "s1", t: 1 }), "utf-8")
