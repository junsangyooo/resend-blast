...
```
scripts/init/             init wizard (npm run init) — generates src/brand.config.ts + .env.local
src/app/                  pages + API routes (auth, send, lists, templates, upload, assets …)
src/components/           UI: send form, tracking, block composer, settings …
src/lib/                  blocks (email renderer), personalize, lists, senders, admins, storage, session …
src/brand.config.ts       white-label config (generated by npm run init)
